...
pub(crate) fn set_emergency_result<A: Encode + TypeInfo + 'static>(
    ...
    Ok(tx(
        pallet_api::election_provider_multi_phase::tx::EMERGENCY,
        vec![scale_result],
    ))
}
...
pub fn signed_solution<S: NposSolution + Encode + TypeInfo + 'static>(
    ...
    Ok(tx(
        pallet_api::election_provider_multi_phase::tx::SUBMIT,
        vec![scale_solution],
    ))
}
...
pub async fn snapshot_at(
    ...
    let addr = storage_addr(
        pallet_api::election_provider_multi_phase::storage::SNAPSHOT,
        empty,
    );
