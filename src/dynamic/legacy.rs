...
type Voters = Vec<(AccountId, VoteWeight, BoundedVec<AccountId, static_types::MaxVotesPerVoter>)>;
...
pub struct State {
	...
	/// We want to drop voters with less VoteWeight, so we're making this heap a min-heap (pop pops
	/// the smallest weight).
