...
fn cli_version_works() {
	...
	let output = assert_cmd::Command::new(cargo_bin(crate_name)).arg("--version").output().unwrap();
...
#[test]
fn westend_dry_run() {
	test_dry_run("westend");
}

fn test_dry_run(chain: &str) {
	let chain_dev = format!("{}-dev", chain);

	let mut cmd = KillChildOnDrop(
		process::Command::new(chain)
			.stdout(process::Stdio::piped())
			.stderr(process::Stdio::piped())
			.args(&["--chain", &chain_dev, "--tmp", "--alice", "--execution", "Native", "--offchain-worker=Always"])
			.spawn()
			.unwrap(),
	);
